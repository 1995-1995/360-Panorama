...
gray_A = double(rgb2gray(image_A));
gray_B = double(rgb2gray(image_B));
...
[hh, ww] = ransacfithomography(matcher_B, matcher_A, npoints, 10);
% drawFeatures(image_A,matcher_A',ww);
% drawFeatures(image_B,matcher_B',ww);
% drawMatched( ww, image_A, image_B, matcher_A', matcher_B' );
% s = load('matcher.mat');
...
firstrowXX=XX(1,:);
lastrowXX=XX(end,:);
firstneg=find(firstrowXX>0);
firstneg=firstneg(1);
lastneg=find(lastrowXX>0);
lastneg=firstneg(1);
slice=max(lastneg,firstneg);
